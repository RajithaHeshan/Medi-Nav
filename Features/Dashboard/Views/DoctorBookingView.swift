...
struct DoctorBookingView: View {
        ...
        VStack(spacing: 0) {
            ...
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 24) {
                    ...
                    // 2. HIG FIX: Add spacer at bottom of scroll content
                    // so the last item isn't cut off by the footer
                    Spacer(minLength: 20)
                }
                .padding()
            }
            
            // 3. HIG FIX: The Sticky Footer
            bottomFooter
        }
        .background(Color(uiColor: .systemGroupedBackground)) // Background for the whole screen
    ...
    private var medicationSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            TextEditor(text: $medicationText).frame(height: 100).padding(8).background(Color.white).clipShape(RoundedRectangle(cornerRadius: 12)).overlay(RoundedRectangle(cornerRadius: 12).stroke(Color.gray.opacity(0.2), lineWidth: 1)).overlay(Group { if medicationText.isEmpty { Text("List your current prescriptions...").font(.caption).foregroundStyle(.gray.opacity(0.6)).padding(.top, 16).padding(.leading, 12) } }, alignment: .topLeading)
        }
    }
    
    // MARK: - 4. HIG COMPLIANT FOOTER
    private var bottomFooter: some View {
        VStack(spacing: 16) {
            ...
            // 5. HIG FIX: Add breathing room below the button
            .padding(.bottom, 10)
        }
        .padding(.horizontal)
        .padding(.top, 16)
