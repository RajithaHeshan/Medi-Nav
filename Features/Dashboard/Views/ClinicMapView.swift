...
struct ClinicMapView: View {
                        ...
                        .overlay(
                            GeometryReader { geometry in
                                ZStack(alignment: .topLeading) {
                                    ...
                                    // 🔴 FIXED: Official SwiftUI method for invisible tap layers
                                    Color.clear
    ...
    // 🔴 FIXED: Added minimumDistance to prevent sloppy taps from triggering drags
    var drag: some Gesture {
    ...
    private func handleMapTap(at location: CGPoint, in size: CGSize) {
        ...
        withAnimation(.spring(response: 0.3, dampingFraction: 0.8)) {
            ...
            // Re-trigger animation cleanly when switching rooms
