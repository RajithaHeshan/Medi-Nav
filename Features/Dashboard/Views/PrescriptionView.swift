...
struct PrescriptionView: View {
    ...
    private var patientInfoCard: some View {
        HStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("Hashani Ranawake").font(.title3).fontWeight(.bold).foregroundStyle(Color(uiColor: .label))
    ...
    private var medicinesSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack {
                Text("Medicines & Dosage").font(.headline).fontWeight(.bold).foregroundStyle(Color(uiColor: .label))
                Spacer()
                Text("3 ITEMS").font(.caption2).fontWeight(.bold).foregroundStyle(.white).padding(.horizontal, 10).padding(.vertical, 4).background(Color(uiColor: .systemBlue)).clipShape(Capsule())
            }
            VStack(spacing: 12) {
    ...
    private var attendingDoctorCard: some View {
        HStack(alignment: .top, spacing: 16) {
            ...
            VStack(spacing: 6) {
                ZStack {
                    RoundedRectangle(cornerRadius: 8).fill(Color(uiColor: .systemGray6)).frame(width: 64, height: 64)
                    Image(systemName: "checkmark.seal.fill").font(.title).foregroundStyle(Color(uiColor: .systemTeal))
                }
    ...
    private var footerActionsView: some View {
        VStack(spacing: 16) {
            ...
            Button { } label: {
                ...
                .font(.headline).fontWeight(.bold).foregroundStyle(Color(uiColor: .label)).frame(maxWidth: .infinity).padding(.vertical, 8)
            }
        }
...
struct PrescriptionMedicineCard: View {
    let medicationName: String; let badgeText: String; let dosage: String; let duration: String; let timing: String; let instruction: String
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack { Text(medicationName).font(.headline).fontWeight(.bold).foregroundStyle(Color(uiColor: .label)); Spacer(); Text(badgeText).font(.caption2).fontWeight(.bold).foregroundStyle(Color(uiColor: .systemBlue)).padding(.horizontal, 10).padding(.vertical, 4).background(Color(uiColor: .systemBlue).opacity(0.1)).clipShape(Capsule()) }
            VStack(spacing: 12) {
                HStack { MedicineInfoLabel(icon: "pills", text: dosage); Spacer(); MedicineInfoLabel(icon: "clock", text: timing) }
                HStack { MedicineInfoLabel(icon: "calendar", text: duration); Spacer(); MedicineInfoLabel(icon: "fork.knife", text: instruction) }
            }
        }
...
struct MedicineInfoLabel: View {
    ...
    var body: some View { HStack(spacing: 8) { Image(systemName: icon).font(.caption).foregroundStyle(Color(uiColor: .systemBlue)).frame(width: 16); Text(text).font(.subheadline).foregroundStyle(Color(uiColor: .secondaryLabel)) }.frame(maxWidth: .infinity, alignment: .leading) }
}
